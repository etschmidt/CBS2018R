...
```
The new price_feature variable shows that Tropicana is inelastic because featuring the it in the weekly brochure made the customers very aware of the price and amplified the price sensitivity.
...
```{r}
# Current Profit
current_profit_unit <- (price - m_cost)*demand
current_profit_total <- current_profit_unit*market_size
cat('Current profit is : $',current_profit_total)
```
```{r}
# Optimize profit
...
opt_profit <- (xmax$maximum-m_cost) * market_size

cat('Optimal price is: $',xmax$maximum)
cat('Optimal profit is : $',opt_profit)
```